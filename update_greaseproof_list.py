...
for prod in products:
    ...
    <div class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
        ...
        <div class="p-5">
            <span class="text-xs font-semibold tracking-wider text-[#2E8B57] bg-[#2E8B57]/10 px-2 py-0.5 rounded-full">防油紙袋</span>
            <h3 class="font-['Noto_Serif_TC',serif] text-lg font-bold mt-2 mb-1">{prod['name']}</h3>
            <p class="text-[#666] text-sm mb-3 line-clamp-2">{prod['description'] if prod['description'] else "專業食品級防油紙袋，安全衛生。"}</p>
            <a href="/zh-tw/products/greaseproof/{slug}" class="text-[#1A5C38] font-semibold text-sm">查看詳情 →</a>
        </div>
    </div>"""
    cards.append(card)
...
<head>
    ...
    <title>防油紙袋 | 壯佳果包裝 - JJG Paper Bag</title>
    <meta name="description" content="壯佳果防油紙袋全系列 — PFAS Free、食品級認證。包含麵包袋、漢堡袋、吐司袋等多款規格。" />
...
<body x-data="{{ mobileMenu: false }}" class="bg-[#F5F0E8] text-[#1A1A1A] font-['Inter','Noto_Sans_TC',sans-serif]">
    ...
    <div class="pt-[76px]">
        <div class="max-w-[1440px] mx-auto px-6 lg:px-10 py-4">
            <nav class="text-sm text-[#666] text-sm">
                ...
                <span class="text-[#1A5C38] font-medium">防油紙袋</span>
            </nav>
        </div>
        ...
        <section class="max-w-[1440px] mx-auto px-6 lg:px-10 pb-12">
            <div class="max-w-3xl">
                ...
                <h1 class="font-['Noto_Serif_TC',serif] text-4xl md:text-5xl font-bold mb-6">防油紙袋</h1>
                <p class="text-[#555] text-lg leading-relaxed mb-6">莊佳果提供最多元化的防油紙袋選擇。我們採用高品質食品級防油紙，透過先進技術打造無 PFAS 且兼具透氣與耐油性的包裝解決方案，完美契合各式餐飲需求。</p>
            </div>
        </section>
        ...
        <section class="bg-white py-16">
            <div class="max-w-[1440px] mx-auto px-6 lg:px-10 text-center">
                <h2 class="font-['Noto_Serif_TC',serif] text-3xl font-bold mb-4">找不到您需要的規格？</h2>
                <p class="text-[#555] text-lg mb-8">我們提供全面的 OEM/ODM 客製化服務，從尺寸、克重到印刷皆可量身打造。</p>
                <a href="/zh-tw/contact.html" class="btn-primary inline-block"><span>聯繫專業團隊</span></a>
            </div>
        </section>
    </div>
...
with open('zh-tw/products/greaseproof.html', 'w', encoding='utf-8') as f:
    f.write(PAGE_TEMPLATE)

print("Updated zh-tw/products/greaseproof.html with all 27 products.")

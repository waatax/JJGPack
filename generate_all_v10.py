...
CATEGORIES = {
    'greaseproof': {
        ...
        'items': [
            {"name": "Bakery Side Window Bag", "images": ["/JJGPack/proimages/detail/bakery-side-window.jpg"], "description": "PE laminated, grease-resistant with high transparency window.", "specs": "Standard bakery side window bag."}
        ] + greaseproof_data
    },
    'kraft': {
        'en': 'Brown Kraft Bag',
        ...
        'items': [
            {"name": "Square Paper Bag", "images": ["/JJGPack/proimages/detail/square-bag.jpg"], "description": "Self-standing design, high-strength long-fiber kraft paper.", "specs": "Standard SOS square bottom bag."},
    ...
    'flat-handle': {
        ...
        'items': [
            {"name": "Flat Handle Bag", "images": ["/JJGPack/proimages/detail/flat-handle-bag.jpg"], "description": "High stability, comfortable grip, food-grade water-based ink.", "specs": "Standard flat handle grocery bag."},
    ...
    'white-kraft': {
        ...
        'items': [
            {"name": "White Kraft Bag", "images": ["/JJGPack/proimages/detail/white-kraft-bag.png"], "description": "Premium white kraft, clean appearance for retail and medical use.", "specs": "High quality white kraft paper."},
    ...
    'aluminum-foil': {
        ...
        'items': [
            {"name": "Aluminum Foil Bag", "images": ["/JJGPack/proimages/detail/aluminum-foil-bag.png"], "description": "Excellent heat retention for roasted chicken and takeaways.", "specs": "Standard aluminum foil lined heat retention bag."},
    ...
    'pe-lamination': {
        ...
        'items': [
            {"name": "L Bag", "images": ["/JJGPack/proimages/detail/l-bag.jpg"], "description": "Leak-proof and grease-resistant burger and sandwich pocket.", "specs": "Standard PE laminated L-bag."},
    ...
    'paper-straw': {
        ...
        'items': [
            {"name": "Paper Straw", "images": ["/JJGPack/proimages/detail/paper-straw.jpg"], "description": "100% Taiwan made, plastic-free, eco-friendly drinking solution.", "specs": "Standard paper straw."},
    ...
    'fruit-protection': {
        ...
        'items': [
            {"name": "Mango Bag", "images": ["/JJGPack/proimages/detail/mango-bag.jpg"], "description": "Double-layer protection from pests and weather for growing fruits.", "specs": "Specially designed for mango protection. Double-layer paper. High pest resistance."}
        ]
    },
    'others': {
        'en': 'Other Products',
        ...
        'items': [
            {"name": "Specialty Bags", "images": ["/JJGPack/proimages/detail/bakery-side-window.jpg"], "description": "Bespoke solutions for unique requirements and custom material needs.", "specs": "Contact us for custom requests."},
...
def gen_category_page(cat_id, cat_data, lang):
    ...
    header = HEADER.format(lang="zh-TW" if is_zh else "en", title=title, home_link=home_link, nav_html=nav_html, en_link=en_link, zh_link=zh_link, en_class=en_class, zh_class=zh_class, contact_link=contact_link, contact_text=contact_text, extra_x_data="")
    cards_html = ""
    for prod in cat_data['items']:
        ...
        cards_html += f'<div class="bg-white rounded-2xl overflow-hidden shadow-premium card-hover group"><a href="{prod_link}" class="aspect-[4/3] overflow-hidden block"><img src="{img}" alt="{prod["name"]}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"></a><div class="p-7"><span class="inline-block text-xs font-bold uppercase tracking-wider text-[#1A5C38] bg-[#1A5C38]/8 px-3 py-1 rounded-full mb-3">{title}</span><h3 class="font-[\'Playfair_Display\',serif] text-xl font-bold mb-2 group-hover:text-[#1A5C38] transition">{prod["name"]}</h3><p class="text-sm text-[#666] mb-5 line-clamp-2">{prod.get("description", "")}</p><a href="{prod_link}" class="inline-flex items-center gap-2 text-[#1A5C38] font-bold text-sm group-hover:gap-3 transition-all">{"查看詳情" if is_zh else "View Details"}<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg></a></div></div>'
    body = f'<section class="max-w-[1440px] mx-auto px-6 py-12"><nav class="text-sm text-[#666] mb-8"><a href="{home_link}">{"首頁" if is_zh else "Home"}</a> › <a href="/JJGPack/{"zh-tw/" if is_zh else ""}products.html">{"產品介紹" if is_zh else "Products"}</a> › <span class="text-[#1A5C38] font-medium">{title}</span></nav><h1 class="font-[\'Playfair_Display\',serif] text-4xl md:text-5xl font-bold mb-4">{title}</h1><p class="text-lg text-[#666] max-w-2xl mb-12">{desc}</p><div class="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{cards_html}</div></section>'
    footer = FOOTER.format(footer_desc="壯佳果股份有限公司 — 台灣專業紙袋製造商，擁有 70 多年的生產經驗。" if is_zh else "Professional paper bag manufacturer in Taiwan with 70+ years of experience.", quick_links="快速連結" if is_zh else "Quick Links", home_text="首頁" if is_zh else "Home", home_link=home_link, products_text="產品介紹" if is_zh else "Products", products_link=f"/JJGPack/{'zh-tw/' if is_zh else ''}products.html", contact_text=contact_text, contact_link=contact_link, footer_products="產品系列" if is_zh else "Products", all_products="所有產品" if is_zh else "All Products", contact_info="聯絡資訊" if is_zh else "Contact Info")
    write_page(f"{'zh-tw/' if is_zh else ''}products/{cat_id}.html", header + body + footer)
...
def gen_product_page(cat_id, prod, lang):
    ...
    header = HEADER.format(lang="zh-TW" if is_zh else "en", title=prod['name'], home_link=home_link, nav_html=nav_html, en_link=en_link, zh_link=zh_link, en_class=en_class, zh_class=zh_class, contact_link=contact_link, contact_text=contact_text, extra_x_data=f", activeImage: '{main_img}'")
    thumbs_html = "".join([f'<button @click="activeImage=\'{img}\'" class="aspect-square rounded-xl overflow-hidden border-2 transition" :class="activeImage===\'{img}\' ? \'border-[#1A5C38] shadow-md\' : \'border-transparent hover:border-gray-200\'"><img src="{img}" class="w-full h-full object-cover"></button>' for img in prod['images']])
    specs = prod.get('specification', prod.get('specs', 'Please contact us for details.' if not is_zh else '請聯繫我們獲取詳情。'))
    body = f'<section class="max-w-[1440px] mx-auto px-6 py-8"><nav class="text-sm text-[#666] mb-10"><a href="{home_link}">{"首頁" if is_zh else "Home"}</a> › <a href="/JJGPack/{"zh-tw/" if is_zh else ""}products.html">{"產品介紹" if is_zh else "Products"}</a> › <a href="/JJGPack/{"zh-tw/" if is_zh else ""}products/{cat_id}.html">{cat_title}</a> › <span class="text-[#1A5C38] font-medium">{prod["name"]}</span></nav><div class="grid lg:grid-cols-2 gap-16 items-start"><div class="space-y-6"><div class="bg-white rounded-3xl overflow-hidden shadow-premium aspect-[4/3] flex items-center justify-center p-4"><img :src="activeImage" alt="{prod["name"]}" class="w-full h-full object-contain"></div><div class="grid grid-cols-4 sm:grid-cols-5 gap-4">{thumbs_html}</div></div><div class="animate-fade-in-right"><span class="inline-block text-xs font-bold uppercase tracking-wider text-[#1A5C38] bg-[#1A5C38]/8 px-4 py-1.5 rounded-full mb-6">{cat_title}</span><h1 class="font-[\'Playfair_Display\',serif] text-4xl md:text-5xl font-bold mb-6">{prod["name"]}</h1><p class="text-lg text-[#555] leading-relaxed mb-10">{prod.get("description", "")}</p><div class="bg-white rounded-2xl p-8 shadow-premium mb-10 border border-gray-100"><h2 class="text-xl font-bold mb-6 flex items-center gap-2"><svg class="w-5 h-5 text-[#1A5C38]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>{"Specifications" if not is_zh else "產品規格"}</h2><div class="text-[#555] whitespace-pre-line leading-relaxed space-y-2">{specs}</div></div><div class="flex flex-wrap gap-4"><a href="{contact_link}" class="btn-primary !px-10"><span>{"Get a Quote" if not is_zh else "索取報價"}</span></a><a href="mailto:export@jjgpaperbag.com" class="btn-outline">{"Email Inquiry" if not is_zh else "郵件諮詢"}</a></div></div></div></section>'
    footer = FOOTER.format(footer_desc="壯佳果股份有限公司 — 台灣專業紙袋製造商，擁有 70 多年的生產經驗。" if is_zh else "Professional paper bag manufacturer in Taiwan with 70+ years of experience.", quick_links="快速連結" if is_zh else "Quick Links", home_text="首頁" if is_zh else "Home", home_link=home_link, products_text="產品介紹" if is_zh else "Products", products_link=f"/JJGPack/{'zh-tw/' if is_zh else ''}products.html", contact_text=contact_text, contact_link=contact_link, footer_products="產品系列" if is_zh else "Products", all_products="所有產品" if is_zh else "All Products", contact_info="聯絡資訊" if is_zh else "Contact Info")
    write_page(f"{'zh-tw/' if is_zh else ''}products/{cat_id}/{slugify(prod['name'])}.html", header + body + footer)

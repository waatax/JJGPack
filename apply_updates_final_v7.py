...
def update_header_content(content, is_zh=False):
    # Desktop Nav
    if is_zh:
    ...
    # Desktop Nav
    new_content = re.sub(r'<div class="hidden lg:flex items-center gap-1">.*?</div>', '<div class="hidden lg:flex items-center gap-1">\n' + links + '\n      </div>', content, flags=re.DOTALL)
    ...
    new_content = re.sub(r'<div class="px-6 py-5 space-y-1">.*?</div>', '<div class="px-6 py-5 space-y-1">\n' + m_links + '\n      </div>', new_content, flags=re.DOTALL)
    
    return new_content

def add_die_cut_bag_item(content, is_zh=False):
...
def create_catalog_html_page(template, is_zh=False):
    catalog_items = [
  ...
  <section class="py-20 bg-white reveal">
    ...
    parts_h = template.split('</header>')
    parts_f = template.split('<!-- ========== FOOTER ========== -->')
    if len(parts_f) < 2: parts_f = template.split('<footer')
    h = parts_h[0] + '</header>'
    f = '<!-- ========== FOOTER ========== -->\n' + parts_f[1]
    return re.sub(r'<title>.*?</title>', f'<title>{"E-Catalog | JJG Paper Bag" if not is_zh else "型錄EDM | 壯佳果包裝"}</title>', h + hero + main + f)
...
def main():
    ...
    for path in all_html:
        ...
        content = update_header_content(content, is_zh)
        if "products.html" in path: content = add_die_cut_bag_item(content, is_zh)
        content = fix_links(content)
        with open(path, 'w', encoding='utf-8') as f: f.write(content)

    with open('catalog.html', 'w', encoding='utf-8') as f: f.write(fix_links(create_catalog_html_page(template_en, False)))
    with open('zh-tw/catalog.html', 'w', encoding='utf-8') as f: f.write(fix_links(create_catalog_html_page(template_zh, True)))
    print("Done.")
